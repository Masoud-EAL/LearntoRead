...
| `options` | an option a learner cannot read can be heard or is a picture or a numeral, and an option that speaks never speaks the answer | "A PLA learner cannot read Goodbye / Please / Good morning, so reading failures are recorded against .08 to .13" |
