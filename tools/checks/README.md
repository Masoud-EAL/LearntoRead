...
| `copying` | a copy with every letter right and the case or a space astray shows the Stage A copy features, .02 Stage A is awarded on it, no run signal is printed above a stage the report does not award, and "Participates" counts answers given | a screenshot of "Working towards PLA.02" for a learner who took part in 44 of 47 questions, with four Stage B lines printed under it |
| `retry` | a wrong, blank or timed out answer at Stage A earns one second ask and only one, the better attempt stands, and the panel records that it took two | "Ask each PLA feature twice. Do it only when the learner cannot answer correctly the first time." |
